...
export function PinPad({ length, filled, onDigit, onDelete, error, onBiometric }: Props) {
  return (
    <div className="flex flex-col items-center">
      {/* dots */}
      <div className={`mb-8 flex gap-3 ${error ? 'animate-[shake_0.35s]' : ''}`}>
...
function Key({
  ...
  return (
    <button
      ...
      onClick={onClick}
      {...rest}
      className="grid size-16 place-items-center rounded-full bg-ink-800/70 font-[family-name:var(--font-display)] text-2xl text-mist-100 transition-colors active:bg-ink-600"
    >
      {children}
    </button>
  )
...
function FaceIcon() {
  return (
    <svg viewBox="0 0 24 24" className="size-7 text-flow" fill="none" stroke="currentColor" strokeWidth={1.7}>
